...
# Ensure GIT_PAGER is disabled for unattended operation
export GIT_PAGER=""

# Read the JSON data from repos.json
json_data=$(cat repos.json)

# Extract the team names and repositories, and set the custom_property 'team_name' for each repo
echo "$json_data" | jq -r 'to_entries[] | "\(.key) \(.value[])"' | while read -r team repo; do
  ...
  gh api --method PATCH -H "Accept: application/vnd.github+json" \
    ...
    -f properties='[{"property_name":"team_name","value":"'"$team"'"}]' --silent
